    ...
    >
      <el-table-column
        ...
        <template v-slot="scope">
          <!-- $event：组件内部触发事件时候传入的值，即需要checkbox的选中状态 -->
          <el-checkbox
            size="mini"
      ...
      <el-table-column
        ...
        label="数量">
        <template v-slot="scope">
          <el-input-number size="mini" :min="1" v-model="scope.row.count" @change="updateProduct({prodId:scope.row.id, count: $event})"></el-input-number>
        </template>
      </el-table-column>
      <el-table-column
        prop="totalPrice"
...
export default {
  ...
  methods: {
    ...mapMutations('cart', ['updateAllProductChecked', 'updateProductChecked', 'deleteFromCart', 'updateProduct'])
  }
}
</script>
